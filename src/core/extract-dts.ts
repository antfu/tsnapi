...
function processExportNamedDeclaration(
  ...
  if (decl) {
    ...
    for (const spec of stmt.specifiers) {
      ...
      if (resolved) {
        const { text, kind } = extractResolvedDeclaration(s, resolved, exportedName, isTypeExport, typeWiden)
...
function resolveFromChunkDts(
  ...
  const { text, kind } = extractResolvedDeclaration(chunkS, resolved, exportedName, isTypeExport, typeWiden)
