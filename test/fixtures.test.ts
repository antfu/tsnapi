...
function readSnap(fixture: string, name: string): string {
  // Normalize CRLF -> LF so assertions are line-ending agnostic. Git may check
  // out committed snapshots with CRLF on Windows, matching tsnapi's own
  // normalization when it reads snapshots for comparison.
  return readFileSync(join(snapshotDir(fixture), name), 'utf-8').replace(/\r\n/g, '\n')
}
